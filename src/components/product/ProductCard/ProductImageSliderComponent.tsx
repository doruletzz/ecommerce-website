...
const ProductImageSliderComponent = ({ product }: Props) => {
	const [imgIndex, setImgIndex] = useState(0);
	const imgProps = useNextImage(product.image[imgIndex]);
	const [direction, setDirection] = useState<'left' | 'right'>();

	return (
		<div className='group w-full flex-1 object-cover relative pb-16 group-hover:pb-0 transition-all duration-[1300ms] ease-in-out-expo'>
			...
			<Link href={`/product/${product.slug.current}`}>
				<div className='w-full h-full object-cover rounded-xl relative -z-10 overflow-hidden'>
					<Image
